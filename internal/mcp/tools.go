...
func Register(srv *sdkmcp.Server, deps *Deps) (err error) {
	...
	// cannot reliably locate a record — runespace exposes no exact-ID retrieval,
	...
	mustAdd(srv, "diagnostics",
		"Collect a 7-section health snapshot (env / state / vault / keys / pipelines / embedding / runespace).",
	...
	mustAdd(srv, "reload_pipelines",
		"Re-initialize Vault + runespace pipelines (BOOT replay) with runespace warmup.",
